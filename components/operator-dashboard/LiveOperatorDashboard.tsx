...
function mapMessage(item: ApiMessage): Message {
  return { id: item.id, sender: item.role === "customer" ? "customer" : item.role === "operator" ? "operator" : "system", body: item.content, time: formatTime(item.createdAt) };
}
...
export function LiveOperatorDashboard() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`/api/conversations/${encodeURIComponent(selectedId)}/messages`, { headers: { "x-operator-token": token } })
      .then(async (response) => {
  ...
  async function sendMessage(body: string) {
    ...
    setConversations((current) => current.map((item) => item.id === selected.id ? {
      ...item,
      messages: [...item.messages, mapMessage(data.message)],
      preview: body,
      status: "対応中",
      updatedAt: formatTime(new Date().toISOString()),
      operatorMessageCount: (item.operatorMessageCount ?? 0) + 1,
      needsOperator: true,
    } : item));
  }
  ...
  async function markResolved() {
    ...
    if (!response.ok) {
      setError("解決済みに変更できませんでした");
      return;
    }
    setConversations((current) => current.map((item) => item.id === selected.id ? { ...item, status: "解決済み", updatedAt: formatTime(new Date().toISOString()) } : item));
    setResolveNotice("解決済みにしました");
  }
